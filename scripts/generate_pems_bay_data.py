#!/usr/bin/env python3
"""
Generate PEMS-BAY dataset CSV files from existing raw files:
1. data/PEMS-BAY.csv from data/pems-bay/pems-bay.h5
2. data/PEMS-BAY-META.csv (sensor_id, latitude, longitude)
3. data/adj_mx_bay.pkl (normalized for Linux unpickling)

Usage:
    python scripts/generate_pems_bay_data.py
    python scripts/generate_pems_bay_data.py --data-dir data
"""
...
EMBEDDED_LOCATIONS = """400001,37.364085,-121.901149
...
def load_h5_data(h5_path: Path) -> pd.DataFrame:
    """Load traffic DataFrame from HDF5 file."""
    print(f"[*] Reading HDF5 data from {h5_path}...")
    try:
        df = pd.read_hdf(h5_path)
        print(f"    Successfully read with pandas: shape={df.shape}")
        return df
    except Exception as e:
        print(f"    pd.read_hdf failed ({e}), attempting direct h5py read...")
        import h5py
        with h5py.File(h5_path, "r") as f:
            speed_group = f["speed"]
            values = speed_group["block0_values"][:]
            timestamps = pd.to_datetime(speed_group["axis1"][:])
            sensor_ids = [
                s.decode("utf-8") if isinstance(s, bytes) else str(s)
                for s in speed_group["axis0"][:]
            ]
        df = pd.DataFrame(values, index=timestamps, columns=sensor_ids)
        print(f"    Successfully read with h5py: shape={df.shape}")
        return df


def generate_pems_bay_csv(df: pd.DataFrame, target_paths: list):
    """Save speed DataFrame as PEMS-BAY.csv."""
    for out_path in target_paths:
        out_path.parent.mkdir(parents=True, exist_ok=True)
        print(f"[*] Writing {out_path} ({len(df)} rows x {df.shape[1]} sensors)...")
        df.to_csv(out_path, index=True, index_label="timestamp")
        print(f"    Saved: {out_path} ({out_path.stat().st_size / 1024 / 1024:.2f} MB)")


def get_sensor_locations(h5_sensors: list) -> pd.DataFrame:
    """Fetch sensor locations from official DCRNN repo, or fallback to embedded data."""
    records = []
    fetched_from_web = False

    print("[*] Fetching sensor coordinates for PEMS-BAY-META.csv...")
    try:
        req = urllib.request.Request(DCRNN_LOCATIONS_URL, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=5) as resp:
            content = resp.read().decode("utf-8").strip().splitlines()
            for line in content:
                parts = line.strip().split(",")
                if len(parts) >= 3:
                    records.append((parts[0].strip(), float(parts[1].strip()), float(parts[2].strip())))
        fetched_from_web = True
        print(f"    Fetched {len(records)} sensor coordinates from DCRNN repository.")
    except Exception as e:
        print(f"    Remote fetch failed ({e}). Using embedded coordinates.")

    if not records:
        for line in EMBEDDED_LOCATIONS.strip().splitlines():
            parts = line.strip().split(",")
            if len(parts) >= 3:
                records.append((parts[0].strip(), float(parts[1].strip()), float(parts[2].strip())))
        print(f"    Loaded {len(records)} embedded sensor coordinates.")

    meta_df = pd.DataFrame(records, columns=["sensor_id", "latitude", "longitude"])
    return meta_df


def generate_pems_bay_meta(meta_df: pd.DataFrame, target_paths: list):
    """Save metadata DataFrame as PEMS-BAY-META.csv."""
    for out_path in target_paths:
        out_path.parent.mkdir(parents=True, exist_ok=True)
        print(f"[*] Writing {out_path} ({len(meta_df)} sensors)...")
        meta_df.to_csv(out_path, index=False)
        print(f"    Saved: {out_path}")


def normalize_adjacency_matrix(in_pkl: Path, target_paths: list):
    """Normalize adj_mx_bay.pkl for Linux cross-platform loading."""
    if not in_pkl.exists():
        print(f"[!] Warning: Adjacency file {in_pkl} not found, skipping.")
        return

    print(f"[*] Normalizing adjacency matrix from {in_pkl}...")
    try:
        with open(in_pkl, "rb") as f:
            raw = f.read().replace(b'\r\n', b'\n')
            data = pickle.loads(raw, encoding="latin1")
    except Exception as e:
        print(f"[!] Failed to unpickle {in_pkl}: {e}")
        return

    for out_path in target_paths:
        out_path.parent.mkdir(parents=True, exist_ok=True)
        if out_path.resolve() == in_pkl.resolve():
            # If overwriting in-place, only write if normalization was needed
            continue
        print(f"[*] Writing normalized adjacency matrix to {out_path}...")
...
def main():
    parser = argparse.ArgumentParser(description="Generate PEMS-BAY CSV files from existing raw files.")
    parser.add_argument("--root-dir", type=str, default=".", help="Project root directory (default: current dir)")
    parser.add_argument("--data-dir", type=str, default="data", help="Target data directory (default: data)")
    parser.add_argument("--h5-file", type=str, default=None, help="Path to pems-bay.h5 (optional)")
    parser.add_argument("--adj-file", type=str, default=None, help="Path to adj_mx_bay.pkl (optional)")
    args = parser.parse_args()

    root = Path(args.root_dir).resolve()
    data_dir = (root / args.data_dir).resolve()
    pems_bay_dir = data_dir / "pems-bay"

    # Locate source h5 file
    if args.h5_file:
        h5_path = Path(args.h5_file).resolve()
    else:
        candidates = [
            pems_bay_dir / "pems-bay.h5",
            data_dir / "pems-bay.h5",
        ]
        h5_path = next((p for p in candidates if p.exists()), None)

    if not h5_path or not h5_path.exists():
        print(f"[ERROR] Could not find pems-bay.h5! Checked: {candidates}")
        sys.exit(1)

    # Locate source adj_mx_bay.pkl
    if args.adj_file:
        adj_path = Path(args.adj_file).resolve()
    else:
        adj_candidates = [
            pems_bay_dir / "adj_mx_bay.pkl",
            data_dir / "adj_mx_bay.pkl",
        ]
        adj_path = next((p for p in adj_candidates if p.exists()), None)

    print("=" * 65)
    print("PEMS-BAY Dataset Preparation Script")
    print("=" * 65)
    print(f"Root directory:      {root}")
    print(f"Data directory:      {data_dir}")
    print(f"Source HDF5 file:    {h5_path}")
    print(f"Source Adj Matrix:   {adj_path}")
    print("=" * 65)

    # 1. Generate PEMS-BAY.csv
    df = load_h5_data(h5_path)
    # Output to both data/ and data/pems-bay/ for compatibility
    csv_targets = [
        data_dir / "PEMS-BAY.csv",
        pems_bay_dir / "PEMS-BAY.csv",
    ]
    generate_pems_bay_csv(df, csv_targets)

    # 2. Generate PEMS-BAY-META.csv
    meta_df = get_sensor_locations(list(df.columns))
    meta_targets = [
        data_dir / "PEMS-BAY-META.csv",
        pems_bay_dir / "PEMS-BAY-META.csv",
    ]
    generate_pems_bay_meta(meta_df, meta_targets)

    # 3. Normalize & Copy adj_mx_bay.pkl to data/
    if adj_path:
        adj_targets = [
            data_dir / "adj_mx_bay.pkl",
        ]
        normalize_adjacency_matrix(adj_path, adj_targets)

    print("=" * 65)
    print("SUCCESS! All required files have been generated:")
    print(f"  1. Traffic speed data: {data_dir / 'PEMS-BAY.csv'}")
    print(f"  2. Metadata table:     {data_dir / 'PEMS-BAY-META.csv'}")
    print(f"  3. Adjacency matrix:   {data_dir / 'adj_mx_bay.pkl'}")
    print("=" * 65)
