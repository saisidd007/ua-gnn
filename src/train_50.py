...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'results/best_model_50.pt')
        ...
        if epoch == num_epochs - 1:
            ...
            os.makedirs('results', exist_ok=True)
            torch.save(checkpoint, 'results/final_checkpoint_50.pt')
    
    # Save training results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs('results', exist_ok=True)
    with open(f'results/training_results_50_{timestamp}.json', 'w') as f:
        json.dump(results, f)
