"""
PEMS-BAY Enhanced Traffic Flow GNN Model with TSSP (Temporal Self-Supervised Prediction)
Replaces Bi-LSTM with TSSP module for temporal dependency modeling
"""
...
class TemporalSelfSupervisedPrediction(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for TSSP block.

        Args:
            x: Node state tensor [num_nodes, hidden_channels]
            edge_index: Graph edge index [2, num_edges]

        Returns:
            Updated node states [num_nodes, hidden_channels]
        """
        residual = x

        # Temporal state update (lightweight, no recurrent loops)
        state_update = self.temporal_update(x)

        # Spatial propagation using normalized adjacency
        num_nodes = x.shape[0]
        device = x.device
...
class TSPPTrafficGNN(nn.Module):
    """
    PEMS-BAY Traffic Flow GNN with TSSP Module
    Features:
    - Multi-layer GCN and GAT combination
    - TSSP (Temporal Self-Supervised Prediction) instead of Bi-LSTM
    ...
    def __init__(
        ...
        super(TSPPTrafficGNN, self).__init__()
        ...
        # Temporal encoder: dilated temporal convolutions with dilations 1,2,4,8
        ...
        self.temporal_norms = nn.ModuleList([
            nn.LayerNorm(hidden_channels) for _ in range(4)
        ])
        ...
        self.tssp = TemporalSelfSupervisedPrediction(
    ...
    def forward(
        ...
        """
        Forward pass with TSSP module
        
        Args:
            x: Input features [num_nodes, in_channels]
            edge_index: Graph connectivity [2, num_edges]
            missing_mask: Boolean mask for missing data
            return_uncertainty: Whether to return uncertainty estimates
            
        Returns:
            predictions, aleatoric_uncertainty, epistemic_uncertainty
        """
        # Handle input shape: if [seq_len, num_sensors], transpose to [num_sensors, seq_len]
        if x.shape[0] < x.shape[1]:
            # Likely [sequence_length, num_sensors], transpose to [num_sensors, sequence_length]
            x = x.t()
        
        # Handle missing data
        x = self.handle_missing_data(x, missing_mask)
        
        # Input projection: [num_sensors, sequence_length] -> [num_sensors, hidden_channels]
        x = self.input_proj(x)
        ...
        # Spatial attention
        x_spatial = x.unsqueeze(0)  # [1, num_sensors, hidden_channels]
        x_spatial = self.spatial_attention(x_spatial)
        x_spatial = x_spatial.squeeze(0)  # [num_sensors, hidden_channels]
        
        # Temporal processing - add batch dimension for temporal blocks
        x_temporal = x.unsqueeze(0)  # [1, num_sensors, hidden_channels]
        for temporal_layer in self.temporal_layers:
            x_temporal = temporal_layer(x_temporal)  # [1, num_sensors, hidden_channels]
        
        # TSSP for temporal dependencies (replaces Bi-LSTM)
        x_temporal = x_temporal.squeeze(0)
        tssp_out = self.tssp(x_temporal, edge_index)  # [num_sensors, hidden_channels]
        x_temporal = tssp_out
        
        # Feature fusion
        combined_features = torch.cat([x_spatial, x_temporal], dim=-1)  # [num_sensors, hidden_channels*2]
        x = self.feature_fusion(combined_features)  # [num_sensors, hidden_channels]
...
def create_tssp_model(
    ...
    """Factory function to create TSSP-based traffic GNN model"""
    return TSPPTrafficGNN(
